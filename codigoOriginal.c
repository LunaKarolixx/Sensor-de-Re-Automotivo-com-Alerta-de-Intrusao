...
void setup_wifi() {
  ...
  WiFi.begin(WIFI_SENHA, WIFI_SENHA);
...
void loop() {
  ...
  // ---------- MODO SEGURANÇA ----------
    ...
    bool intrusaoDetectada = (vibracao_status == LOW || inclinacao_status == LOW);
...
// Função para alternar entre os modos (MANOBRA / SEGURANÇA)
bool alternarModo(bool modoAtual) {
  static bool estadoAnterior = HIGH; // botão começa "solto" por causa do PULLUP
  bool estadoAtual = digitalRead(botao_modo);

  // Detecta transição de HIGH -> LOW (aperto do botão)
  if (estadoAnterior == HIGH && estadoAtual == LOW) {
    ...
    // Estado neutro na transição
    digitalWrite(ledGreen,  LOW);
    digitalWrite(ledYellow, LOW);
    digitalWrite(ledRed,    LOW);
    noTone(buzzer); // Usa noTone para desligar o buzzer

    delay(300);           // pequeno debounce
    estadoAnterior = estadoAtual;
    return novoModo;      // muda o modo só aqui
  }

  estadoAnterior = estadoAtual;
  return modoAtual;       // se não clicou, mantém
}
